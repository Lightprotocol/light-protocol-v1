...
mod tests {
    ...
    fn initialize(tree: &mut merkle_tree, treelevels: usize, mut zero_value: Vec<u8>) {
        ...
        for i in 1..tree.levels {
            ...
            let leaf_hash = <PoseidonCircomCRH3 as TwoToOneCRH>::evaluate(
                &params,
                &tree.zeros[tree.zeros.len() - 1],
                &tree.zeros[tree.zeros.len() - 1],
            )
            .unwrap();
        ...
        let root_tmp = <PoseidonCircomCRH3 as TwoToOneCRH>::evaluate(
            &params,
            &tree.zeros[tree.zeros.len() - 1],
            &tree.zeros[tree.zeros.len() - 1],
        )
    ...
    fn merkle_tree_arkworks_fork_vs_tornado_cash_fork_init() {
        ...
        for i in 1..12 {
            ...
            let leaves: Vec<Vec<u8>> =
                vec![smt.zeros[0].clone(); 2_usize.pow(tree_height.try_into().unwrap())];
    ...
    fn print_init_bytes_helper(smt: merkle_tree) -> Result<(), ioError> {
        ...
        write!(
            ...
            "{}",
            format!(
                "pub const INIT_BYTES_MERKLE_TREE_{} : [u8;{}] = {:?};",
                smt.levels,
                init_bytes.len(),
                init_bytes
            )
        ...
        write!(
            ...
            "{}",
            format!(
                "\n\npub const ZERO_BYTES_MERKLE_TREE_{} : [u8;{}] = {:?};",
                smt.levels,
                zero_bytes.len(),
                zero_bytes
            )
        ...
        // 	instruction_order.push(j);
        // }
        instruction_order.push(16u8);

        write!(
            output,
            "{}",
            format!(
                "\n\npub const INSERT_INSTRUCTION_ORDER_{} : [u8;{}] = {:?};",
                smt.levels,
